...
- Added a new "Field notes" panel to the sidebar that can be used to jot down
  quick notes without having to leave Aerostem Live.
...
- UAVs now support the new "sleep state" status code introduced in Aerostem
  Server 2.10.0.
...
- The graticule layer (the latitude-longitude grid overlaid on top of the map)
  ...
  Aerostem is configured to show latitudes and longitudes using degrees,
...
- You can now provide your own API keys to access Mapbox and Maptiler tile
  services. In the absence of an API key, Aerostem Live falls back to using an
  API key that is shared between all users of Aerostem Live. No guarantees are
...
- Aerostem Server users with the offline maps extension can now benefit from
  ...
  test area from Aerostem Live with the cache enabled and zooming in and out a
...
The source code of Aerostem Live is now licensed under the GNU General Public
...
- Fixed the termination of the server process on macOS when the server is
  launched automatically by Aerostem Live.
...
- Added a new Beacons panel and a Beacons layer to the map view; this layer
  ...
  Aerostem server 1.27.0 and later, this layer will also show the position of
...
- Drones marked as "gone" in the UAV list are now considered "missing" in the
  ...
  or were explicitly removed from Aerostem Live were considered missing.
...
- Aerostem Live prevents the system from going to sleep automatically when it is
