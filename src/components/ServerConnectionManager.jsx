...
/**
 * Component that launches a local Aerostem server instance when mounted.
 */
  ...
  (event) => (dispatch, getState) => {
    ...
    switch (reason) {
      case 'connection error':
        if (!willReconnect) {
          dispatch(showError('Error while connecting to Aerostem server'));
        }
      ...
      case 'connection timeout':
        if (!willReconnect) {
          dispatch(showError('Timeout while connecting to Aerostem server'));
        }

        break;

      case 'io client disconnect':
        dispatch(showNotification('Disconnected from Aerostem server'));
        break;
      ...
      case 'transport close':
        dispatch(showError('Aerostem server closed connection unexpectedly'));
        break;

      case 'ping timeout':
        dispatch(showError('Connection to Aerostem server lost'));
        break;
...
const ServerConnectionManager = connect(
  ...
  (dispatch) => ({
    ...
    onConnected() {
      ...
      dispatch(
        showNotification({
          message: i18n.t('notifications.connectedToAerostemServer'),
          semantics: 'info',
        })
      );
    ...
    onLocalServerError(message, wasRunning) {
      const baseMessage = wasRunning
        ? 'Aerostem server died unexpectedly'
        : 'Failed to launch local Aerostem server';
      dispatch(setCurrentServerConnectionState(ConnectionState.DISCONNECTED));
