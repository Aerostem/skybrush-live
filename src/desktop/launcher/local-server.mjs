...
/**
 * The local executable of the Aerostem server on this machine.
 */
let localServerPath;

/**
 * Deferred that will resolve to the local executable of the Aerostem server
 * on this machine.
 */
let localServerPathDeferred = pDefer();

/**
 * The process representing the launched Aerostem server instance on this
 * machine. We only support one Aerostem server instance per Aerostem Live
 * process.
 */
 ...
 *         Aerostem server executable, besides the system path and the
...
function getPathsRelatedToAppLocation() {
  ...
  // named "Aerostem Server". This allows us to find it when the server and
  ...
  if (isWindows) {
    const SERVER_FOLDER_NAME = 'Aerostem Server';
    const rootFolders = [
...
const deriveServerPathAndArgumentsFromOptions = async (options) => {
  ...
  if (!localServerPath) {
    throw new Error('local Aerostem server not found');
  }
...
/**
 * Ensures that a Aerostem server executable with the given arguments is up and
...
/**
 * Launches the local Aerostem server executable with the given arguments.
 *
 * @param {Object}   options         options to tweak how the server is launched
 * @param {string[]} options.args    additional arguments to pass to the server
 *        when launching
...
/**
 * Searches for the local Aerostem server executable in the following places,
...
export const selectPath = async (defaultPath, browserWindow) => {
  const options = {
    title: 'Select directory containing Aerostem Server',
